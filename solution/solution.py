...
def call_openai(
    ...
    if not api_key:
        raise ValueError("Chưa cấu hình biến môi trường OPENAI_API_KEY")
    client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"))
...
def count_tokens(text: str, model: str = OPENAI_MODEL) -> int:
    ...
    try:
        ...
        # Fallback estimation if offline or if the model name isn't recognized by tiktoken
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        # Keep only the last 3 turns (up to 6 messages)
        ...
        try:
            ...
            print(f"\n[Error]: {e}")
            # Remove the last unhandled user message if the call failed
            history.pop()
...
def run_assistant(
    ...
    while True:
        # Stop if max_turns limit is reached
        if max_turns is not None and num_turns >= max_turns:
            break
        ...
        # Build payload: System prompt + Conversation history + New User message
        messages = (
        ...
        # Execute API call wrapped inside retry_with_backoff
        try:
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    def truncate(text: str, max_len: int = 40) -> str:
        # Thay thế xuống dòng bằng khoảng trắng để hiển thị gọn trên 1 dòng
    ...
    # Tạo danh sách dữ liệu đã xử lý cắt chuỗi / định dạng
    rows = []
    for item in results:
        p = truncate(item.get("prompt", ""))
        g_resp = truncate(item.get("gpt4o_response", ""))
        m_resp = truncate(item.get("mini_response", ""))
        g_lat = f"{item.get('gpt4o_latency', 0.0):.2f}s"
        m_lat = f"{item.get('mini_latency', 0.0):.2f}s"
        rows.append([p, g_resp, m_resp, g_lat, m_lat])

    # Tính độ rộng cột tối thiểu dựa trên header và dữ liệu
    col_widths = [
        max(len(header), max((len(row[i]) for row in rows), default=0))
        for i, header in enumerate(headers)
    ]

    # Hàm tạo dòng kẻ bảng
    def make_row(cols):
